...
export default function WinnerPopup({
  ...
  const handleRejoin = () => {
    if (player.player) {
      socket.emit("join_room", player.room_id);
    }
    const name_roomID = {
      p_id: socket.id,
  ...
  return (
    <div className="text-center">
      <button
        className="rounded-l-full bg-yellow-500 text-white p-2 text-3xl"
      ...
      >
        <DialogTitle>
          <div className="lg:text-4xl sm:text-2xl">
            The winner is <b>{body[0].winner}</b>
          </div>
        </DialogTitle>
        <DialogContent>
